...
router.get('/api/inspecciones/', (req, res) => {
  mysqlConnection.query('SELECT UNICODE, FECHA, STATUS_INSPECCION, INTRA_CHIRIS, PERI_CHIRIS FROM INSPECCIONES', (err, rows, fields) => {
    if(!err) {
      res.json(rows);
    } else {
      console.log(err);
